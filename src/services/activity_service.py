...
ACTIVITY_REQUIREMENTS: Dict[ActivityType, Tuple[List[str], Optional[str], Optional[str], Optional[str], Optional[str], Optional[str]]] = {
    ActivityType.FIRST_CONTACT: ([], 'conversation', "YAWN2", None, None, None),
...
class ActivityService(BaseService):
    ...
    async def handle_event(self, event: Dict[str, Any]):
        ...
        if event_type == "application_startup_completed":
            ...
            elif intent == "system_exit_app":
                self.logger.info("Exit app intent received. Stopping application.")
                await self.publish({"type": "speak_audio", "text": "OK, goodbye."})
                await exit_app()
